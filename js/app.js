...
function enterApp(){
  ...
  var rs=document.getElementById('menu-reset-stats');
  if(rs)rs.style.display=session.isAdmin?'block':'none';
...
function _tsToISO(ts){if(!ts)return '';var d=new Date(ts);var mm=('0'+(d.getMonth()+1)).slice(-2),dd=('0'+d.getDate()).slice(-2);return d.getFullYear()+'-'+mm+'-'+dd;}
  ...
  try{
    ...
    if(wb.SheetNames.indexOf('Assistencies_Tecniques')>=0){
      ...
      plannerRows=mA.slice(hiA+1).map(function(cols){
        ...
        row.__ts=d?d.getTime():null;row.__date=d?_tsToInput(d.getTime()):'';row.__iso=d?_tsToISO(d.getTime()):'';
        return row;
...
async function confirmAssistImport(){
  ...
  inR.forEach(function(row){
    ...
    var newM={id:'assist_'+Date.now()+'_'+imported,name:desc||'Assistència tècnica',desc:(subj?('Subjecte: '+subj):''),arc:arc,playerId:assignedPlayer?assignedPlayer.id:'',status:status,diff:'C',xp:MISSION_XP,gold:gold,frag:MISSION_FRAG,durationH:hours,stars:stars,attr:'Intel·ligència',attrPts:2,deadline:row.__iso||'',daily:false,isDaily_instance:false,plannerId:plannerId,createdBy:session.playerId,fromPlanner:true,plannerCreator:tech,plannerAssignee:subj,plannerTags:cat};
    missions.push(newM);
    if(status==='done'){
      if(assignedPlayer){awardMissionTo(assignedPlayer,newM,(row.__iso?row.__iso+'T12:00:00.000Z':null));affected[assignedPlayer.id]=true;autoClaimed++;}
      else{rewardsPending[newM.id]=true;}
    }else{pend++;}
    imported++;
  });
  _suppressLevelUp=false;
  if(CFG.MODE==='supabase')saveToSupabase(Object.keys(affected));
  clearPlannerImport();
...
function confirmPlannerImport(){
  ...
  plannerRows.forEach(function(row){
    ...
    if(status==='done'){
      if(assignedPlayer){awardMissionTo(assignedPlayer,newM);affected[assignedPlayer.id]=true;autoClaimed++;}
      else{rewardsPending[newM.id]=true;}
    }
    imported++;
  });
...
// Reset de proves: torna tots els jugadors als stats base (nivell 1, 0 XP/or/frags/atributs) i buida l'historial
async function resetAllStats(){
  if(!session.isAdmin){toast('Només l\'admin pot fer això.');return;}
  var _x=document.getElementById('umenu-inline');if(_x)_x.style.display='none';
  if(!await uiConfirm('Resetejar els STATS de TOTS els jugadors a zero (nivell 1, 0 XP, 0 or, 0 fragments, 0 atributs, 0 missions) i buidar l\'historial d\'estadístiques?\n\nEs conserven: nom, classe, avatar, inventari, cosmètics i galeria. Les missions NO s\'esborren (per treure les assistències importades usa el seu botó).\n\nAixò no es pot desfer (excepte restaurant una còpia).'))return;
  var typed=await uiPrompt('Escriu RESET per confirmar:','');
  if((''+(typed||'')).trim().toUpperCase()!=='RESET'){toast('Cancel·lat.');return;}
  var xpNext=(typeof XP_PER_LEVEL!=='undefined'?XP_PER_LEVEL:100);
  players.forEach(function(p){
    p.level=1;p.xp=0;p.xpNext=xpNext;
    p.gold=0;p.goldMonth=0;p.goldMonthKey='';
    p.fragments=0;p.gachaTokens=0;p.missions=0;p.pendingAttrPts=0;
    p.attrs=zeroAttrs();p.baseAttrs=zeroAttrs();
    p.itemAcq={};
  });
  statsLog=[];
  rewardsPending={};
  ...
  renderAll();
  try{renderHeroProfile(curHero);}catch(e){}
  try{if(document.getElementById('page-panoramica').classList.contains('active'))renderPanoramica();}catch(e){}
  toast('Stats resetejats. Tot a zero per fer proves.');
}
async function clearImportedAssist(){
  var ids=missions.filter(function(m){return m.plannerId&&(''+m.plannerId).indexOf('assist_')===0;}).map(function(m){return m.id;});
  if(!ids.length){toast('No hi ha assistències importades.');return;}
  if(!await uiConfirm('Esborrar '+ids.length+' assistències importades? Podràs tornar-les a importar. (No descompta l\'or/XP ja donats.)'))return;
  missions=missions.filter(function(m){return !(m.plannerId&&(''+m.plannerId).indexOf('assist_')===0);});
  ids.forEach(function(id){delete rewardsPending[id];delete missionAssignees[id];});
  if(CFG.MODE==='supabase'){
    for(var i=0;i<ids.length;i++){try{await deleteMissionFromSupabase(ids[i]);}catch(e){}}
    await saveToSupabase();
  }
  renderAll();try{renderPlannerImported();}catch(e){}
  toast(ids.length+' assistències esborrades. Ja pots tornar a importar-les.');
}
